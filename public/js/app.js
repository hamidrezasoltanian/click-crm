...
function openDailyMonitor(){
  ...
  Object.keys(DB.weekEntries||{}).forEach(function(k){
    ...
    todayEntries.push({key:k,rtype:rtype,rid:rid,name:name,owner:owner,actType:we.actionType||'call',status:e.status||'بدون تماس',activities:acts});
  });
  ...
  var totalCenters=todayEntries.length;
  var activeCenters=todayEntries.filter(function(en){return en.activities.length>0;}).length;
  var inactiveCenters=totalCenters-activeCenters;

  var body='<div style="margin-bottom:14px;display:flex;gap:12px;flex-wrap:wrap">'
    +'<div style="background:#f0fdf4;border:1px solid #86efac;border-radius:8px;padding:8px 16px;text-align:center"><div style="font-size:20px;font-weight:800;color:#16a34a">'+totalCenters+'</div><div style="font-size:11px;color:#166534">مرکز امروز</div></div>'
    +'<div style="background:#f0fdf4;border:1px solid #86efac;border-radius:8px;padding:8px 16px;text-align:center"><div style="font-size:20px;font-weight:800;color:#16a34a">'+activeCenters+'</div><div style="font-size:11px;color:#166534">با فعالیت</div></div>'
    +'<div style="background:#fef2f2;border:1px solid #fca5a5;border-radius:8px;padding:8px 16px;text-align:center"><div style="font-size:20px;font-weight:800;color:#dc2626">'+inactiveCenters+'</div><div style="font-size:11px;color:#991b1b">بدون گزارش</div></div>'
    +'</div>';

  if(!totalCenters){
    body+='<div style="text-align:center;padding:40px;color:var(--text-muted)"><div style="font-size:32px;margin-bottom:10px">\u{1F4CB}</div><div>هیچ مرکزی برای امروز برنامه‌ریزی نشده</div></div>';
  } else {
    Object.keys(byOwner).sort().forEach(function(ow){
      var entries=byOwner[ow];
      var owName=ow==='__none__'?'بدون مسئول':(USERS[ow]||ow);
      var owActive=entries.filter(function(en){return en.activities.length>0;}).length;
      body+='<div class="dm-section">'
        +'<div class="dm-expert-head">'
        +'<span>\u{1F464} '+esc(owName)+'</span>'
        +'<span style="font-size:11px;color:var(--text-secondary)">'+entries.length+' مرکز — '+owActive+' فعال / '+(entries.length-owActive)+' بدون گزارش</span>'
        +(ow!=='__none__'&&(entries.length-owActive)>0?'<button onclick="sendReminderToExpert(\''+ow+'\')" style="margin-right:auto;background:#fef3c7;color:#92400e;border:1px solid #fcd34d;border-radius:5px;padding:2px 10px;cursor:pointer;font-size:10px;font-family:inherit">\u{1F4E9} یادآوری</button>':'')
        +'</div>';
      entries.forEach(function(en){
        var hasAct=en.activities.length>0;
        var actTypeLabel=en.actType==='visit'?'\u{1F91D} ویزیت':'\u{1F4DE} تماس';
        body+='<div class="dm-center-row">'
          +'<div class="dm-center-name">'+esc(en.name)+'</div>'
          ...
          +'<span class="dm-status-badge" style="background:var(--bg-raised);border:1px solid var(--border)">'+esc(en.status)+'</span>'
          +(hasAct
            ?'<span class="dm-act-chip">✓ فعال</span>'
            :'<span class="dm-noact-chip">✗ بدون گزارش</span>')
          +(hasAct?'<div class="dm-changes-list">'+en.activities.slice(0,5).map(function(a){return _fieldLabel(a.field)+': '+esc(String(a.val||''));}).join(' &nbsp;|&nbsp; ')+'</div>':'')
          +'</div>';
      });
      body+='</div>';
    });
  ...
  var foot='<button class="btn-secondary" onclick="closeModal(\'dmModal\')">بستن</button>'
    +'<button class="btn-primary" onclick="openDailyMonitor()">\u{1F504} بروزرسانی</button>';
  openModal('dmModal','\u{1F4CA} گزارش فعالیت روزانه — '+today,body,foot,{xl:true});
}
...
function _setupAutoReminder(){
  // یادآوری ساعت ۳ برای مراکز امروز: هر ۶۰ ثانیه چک
  setInterval(function(){
    if(_autoReminderChecked) return;
    if(!_isManager()) return;
    var now = new Date();
    if(now.getHours() < 15) return;
    var today = todayStr();
    if((DB._lastAutoReminder||'') === today) return;
    _autoReminderChecked = true;
    DB._lastAutoReminder = today;
    saveDB();
...
function _runTodayReminders(today){
  ...
  Object.keys(byExpert).forEach(function(exp){
    ...
    var msg = 'یادآوری ساعت ۱۵: این مراکز امروز گزارش نشده‌اند: '
      + items.slice(0,5).map(function(x){return x.name;}).join('، ')
      + (items.length>5?' و '+(items.length-5)+' مرکز دیگر':'');
...
function _runOverdueAndUndatedReminders(today){
  var byExpert = {}; // expert → {noDate:[], overdue:[]}
  _buildPCCache();
  Object.keys(DB.weekEntries||{}).forEach(function(k){
    var we = DB.weekEntries[k];
    if(we.done || we.rtype === 'mtr') return;
    ...
    if(!we.scheduledDate){
      ...
      // تاریخ گذشته: بررسی آیا گزارشی در اون روز داشته
  ...
  Object.keys(byExpert).forEach(function(exp){
    ...
    var msgs = [];
    if(d.noDate.length){
      msgs.push('📅 '+d.noDate.length+' مرکز بدون تاریخ در برنامه هفته: '
        + d.noDate.slice(0,4).map(function(x){return x.name;}).join('، ')
        + (d.noDate.length>4?' و '+(d.noDate.length-4)+' مرکز دیگر':''));
    }
    if(d.overdue.length){
      msgs.push('⚠ '+d.overdue.length+' مرکز با تاریخ گذشته بدون گزارش: '
        + d.overdue.slice(0,4).map(function(x){return x.name+' ('+x.date+')';}).join('، ')
        + (d.overdue.length>4?' و '+(d.overdue.length-4)+' مرکز دیگر':''));
    }
    if(!msgs.length) return;
    cnt++;
    var firstKey = (d.overdue[0]||d.noDate[0]||{}).key||'';
    sendNotif(exp, msgs.join(' | '), firstKey);
  });
