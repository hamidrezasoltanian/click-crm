...
(async () => {
  ...
  // Let's get columns of Factor table
  ...
  // Let's get sample of 10 factors to see their data, including column names like Type, Name, Date, CustomerNum, etc.
  const sampleRes = await pool.request().query(`
    SELECT TOP 10 * FROM Factor
  `);
  console.log('\nFactor Sample Data:');
